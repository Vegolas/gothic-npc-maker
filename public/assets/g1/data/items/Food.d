...
const int	Value_Hallenpilz	=	 3;		const int	HP_Hallenpilz		=	 6;
...
// FIXME: zs_Drunk fÃ¼r SC oder nicht? 
...
{	
	...
	description			= name;
	TEXT[1]				= NAME_Bonus_HP;				COUNT[1]	= HP_Hallenpilz;
	TEXT[5]				= NAME_Value;					COUNT[5]	= Value_Hallenpilz;
};

		func void Usemush ()
		{
			Npc_ChangeAttribute	(self,	ATR_HITPOINTS,	HP_Hallenpilz);
			PrintDebugNpc 		(PD_ITEM_MOBSI, "Ich esse HÃ³llenpilz");
		};
...
{	
	...
	func void UsePlants1 ()
	{
		...
		PrintDebugNpc 		(PD_ITEM_MOBSI, "Ich esse HeilkrÃ¤uter");
	};

/******************************************************************************************/
INSTANCE ItFo_Plants_Herb_02(C_Item)
{	
	name 				=	"Haszcz leczäncy";
...
instance ItFo_SG_Zur(C_ITEM)
{
	...
	description			= name;
	TEXT[1]				= "¯ur zrobiony przez Neka. Nawet fajnie wonio.";

	if(ZUR_USED == 0)
	{
		TEXT[3]				= NAME_Bonus_HP;				COUNT[3]	= 10;
	}
	else
	{
		TEXT[3]				= NAME_Bonus_HP;				COUNT[3]	= HP_SGFOOD;
	};
	...
	func VOID UseZur()
	{
		...
		{
			Npc_ChangeAttribute	(self,	ATR_HITPOINTS,	HP_SGFOOD);
		};
	};

/***********************/
instance ItFo_SG_Ciapkapusta(C_ITEM)
{
	name 				=	"Ciapkapusta i bergi";
	...
	visual 				=	"sg_food_ciapkapusta.3ds";
	material 			=	MAT_WOOD;
	on_state[0]			= 	UseCiapkapusta;
	scemeName			= 	"FOOD";

	description			= name;
	TEXT[1]				= "Nieftorzy pedzieliby, ¿e jednak karminadle.";

	if(CIAPKAPUSTA_USED == 0)
	{
		TEXT[3]				= NAME_Bonus_STR;				COUNT[1]	= 5;
	}
	else
	{
		TEXT[3]				= NAME_Bonus_HP;				COUNT[3]	= HP_SGFOOD;
...
instance ItFo_SG_Wodzionka(C_ITEM)
{
	name 				=	"Wodzionka";
	...
	visual 				=	"ItFo_Meatbugragout_01.3ds";
	material 			=	MAT_WOOD;
	on_state[0]			= 	UseWodzionka;
	scemeName			= 	"FOOD";

	description			= name;
	TEXT[1]				= "To nojlepszo ze wszyskich zup.";
	if(SIEMIOTKA_USED == 0)
	{
		TEXT[3]				= NAME_Bonus_DEX;				COUNT[3]	= 5;
	}
	else
	{
		TEXT[3]				= NAME_Bonus_HP;				COUNT[3]	= HP_SGFOOD;
...
instance ItFo_SG_Krupniok(C_ITEM)
{
	name 				=	"Krupniok";
	...
	visual 				=	"sg_food_krupniok.3ds";
	material 			=	MAT_WOOD;
	on_state[0]			= 	UseKrupniok;
	scemeName			= 	"FOOD";

	description			= name;
	TEXT[1]				= "Fajnie usma¿äny, a¿ co³y corny.";
	if(KRUPNIOK_USED == 0)
	{
		TEXT[3]				= NAME_Bonus_HP;				COUNT[3]	= 10;
	}
...
instance ItFo_SG_Siemiotka(C_ITEM)
{
	name 				=	"Siemiotka";
	...
	visual 				=	"ItFo_Crawlersoup_01.3ds";
	material 			=	MAT_WOOD;
	on_state[0]			= 	UseSiemiotka;
	scemeName			= 	"FOOD";

	description			= name;
	TEXT[1]				= "Tak bez ¿odnego œwiynta zrobiono? A¿ dziwne.";
	...
	func VOID UseSiemiotka()
	{
		if(SIEMIOTKA_USED == 0)
		{
			B_RaiseAttribute(ATR_MANA_MAX, 10);
			SIEMIOTKA_USED = 1;
		}
		else
		{
...
instance ItFo_SG_Rolady(C_ITEM)
{
	name 				=	"Rolady, modro kapusta i gumiklyjze.";
	...
	visual 				=	"sg_food_rolada.3ds";
	material 			=	MAT_WOOD;
	on_state[0]			= 	UseRolady;
	scemeName			= 	"FOOD";

	description			= name;
	TEXT[1]				= "Porzändny, Œlänski ôbiod.";
	if(ROLADY_USED == 0)
	{
		TEXT[3]				= "Bonus P¯ i many:";				COUNT[3]	= 15;
		TEXT[4]				= "Bonus siy³y i grajfki";			COUNT[4]	= 5;
	}
	else
	{
		TEXT[3]				= NAME_Bonus_HP;				COUNT[3]	= HP_SGFOOD;
	};
	TEXT[5]				= NAME_Value;						COUNT[5]	= 350;
};
